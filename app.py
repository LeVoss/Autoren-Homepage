...
# 3. Horizontales Menü (Perfekt aufgeteilt für 4 Buttons)
...
with menu_col2:
    if st.button("👤 Über mich", use_container_width=True):
        st.switch_page("pages/1_Über_mich.py")
with menu_col3:
    if st.button("📅 Termine", use_container_width=True):
        st.switch_page("pages/2_Veranstaltungen.py")
with menu_col4:
    if st.button("🎬 Media", use_container_width=True):
        st.switch_page("pages/3_Multimedia.py")
...
with col4:
    ...
    **Klappentext:**
    ...
    # Amazon-Link für dein 1. Buch "Ein leises wir"
    ...
    st.markdown(f"📖 **[Auf Amazon ansehen & bestellen]({amazon_url_buch1})**")
